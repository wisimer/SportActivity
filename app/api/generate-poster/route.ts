...
export async function POST(request: NextRequest) {
  try {
    ...
    let prompt = `根据这张图片，制作一张Q版3D卡通风格的大头表情包图片，使用里面人物的面貌特征，保留脸部和发型特征，
    人物的服装更换为更合适${sportType}的搭配。图片右上角写着“${sportType}”的字。
    人物姿势为${sportType}运动的动作。
    添加符合项目的环境元素（如泳池波浪线/田径跑道），并且只有一个人物角色，不要出现其他人物角色。`;
